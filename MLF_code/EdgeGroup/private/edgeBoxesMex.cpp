...
public:
  ...
  void generate( Boxes &boxes, arrayf &E, arrayf &O, arrayf &V, arrayi &V_id);
...
private:
  ...
  // helper routines
  void clusterEdges( arrayf &E, arrayf &O, arrayf &V, arrayi &V_id);
...
void EdgeBoxGenerator::generate( Boxes &boxes, arrayf &E, arrayf &O, arrayf &V, arrayi &V_id)
{
  clusterEdges(E,O,V,V_id); prepDataStructs(E); scoreAllBoxes(boxes);
}

void EdgeBoxGenerator::clusterEdges( arrayf &E, arrayf &O, arrayf &V, arrayi &V_id )
{
  ...
  if( V_id._x ) for( c=0; c<w; c++ ) for( r=0; r<h; r++ ) {
    i=_segIds.val(c,r);
    V_id.val(c,r) = i<=0 ? 0 : i;
  }
}
...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  if(nl > 3) mexErrMsgTxt("At most two outputs expected.");
  ...
  arrayi V_id; arrayf V;
  if( nl>1 ) {
    const int ds[3] = {h,w,3};
    pl[1] = mxCreateNumericArray(3,ds,mxSINGLE_CLASS,mxREAL);
    V._x = (float*) mxGetData(pl[1]); V._h=h; V._w=w;

    pl[2] = mxCreateNumericMatrix(h,w,mxINT32_CLASS,mxREAL);
    V_id._x = (int*) mxGetData(pl[2]); V_id._h=h; V_id._w=w;
  }
  ...
  edgeBoxGen.generate( boxes, E, O, V, V_id );
